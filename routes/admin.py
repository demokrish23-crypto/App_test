...
def manage_users():
    ...
    for u in file_users:
        ...
        if u["role"] == "admin" and u["email"] != current_admin_email:
            password = "********"
        else:
            password = u["password"]
...
def delete_user(user_id):
    # Delete user's questions and papers
...
def session_timeline_data():
    ...
    try:
        ...
        # Group by hour and count active sessions
        ...
        if not hours:
            # Fallback: show zero activity if no sessions
...
def delete_database():
    ...
    try:
        ...
        # Optional: Delete the SQLite database file and recreate it
        db_file = os.path.join(current_app.instance_path, 'app.db')
        if os.path.exists(db_file):
            os.remove(db_file)
        
        # Recreate tables one more time to ensure clean state
        db.create_all()
        db.session.commit()
